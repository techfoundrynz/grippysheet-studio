...
const GeometryControls: React.FC<GeometryControlsProps> = ({
  ...
  const [libraryPatternName, setLibraryPatternName] = useState<string | null>(null);

  // Re-implement calculation logic locally or import if available
  const calculateAutoPatternScale = (shapes: any[], type: string | null, tiled: boolean, bSize: number, margin: number): number | null => {
  ...
  const handlePatternLoaded = (shapes: any[], type?: 'dxf' | 'svg' | 'stl') => {
      ...
      const newScale = calculateAutoPatternScale(shapes, pType, isTiled, baseSize, patternMargin);
        ...
        <>
            ...
            <SegmentedControl
                ...
                onChange={(val) => {
                    ...
                    const newScale = calculateAutoPatternScale(patternShapes, patternType, newIsTiled, baseSize, patternMargin);
              ...
              <div className="space-y-2 flex-1 min-w-0">
                ...
                >
                    ...
                    onChange={(val) => {
                        ...
                        let updateObject: Partial<GeometrySettings> = { patternScale: newScale };
                        if (patternScaleZ !== '' && patternScale > 0) {
                            const ratio = newScale / patternScale;
                            const newZ = Number(patternScaleZ) * ratio;
                            updateObject.patternScaleZ = Math.round(newZ * 1000) / 1000;
                        }
              ...
              <div className="space-y-2 flex-1 min-w-0">
                   <ControlField label="Scale Z" tooltip="Leave empty to match X/Y scale">
                       <DebouncedInput
                           type="number"
                           value={patternScaleZ}
                           onChange={(val) => updateSettings({ patternScaleZ: val === '' ? '' : Number(val) })}
                           placeholder="Auto"
                           min={0.1}
                           step={0.05}
                           className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all outline-none"
              ...
              <div className="flex-1 min-w-0">
                <ControlField label="Margin" tooltip="Safety margin from edge">
                    <DebouncedInput
                        type="number"
                        value={patternMargin}
                        onChange={(val) => updateSettings({ patternMargin: Number(val) })}
                        step="0.5"
                        min="0"
                        className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all outline-none"
                    />
              ...
              <div className="flex-1 min-w-0">
                <ControlField label="Clip to Edge" tooltip="Trim patterns that cross the outline boundary">
                    <ToggleButton
                        ...
                        onToggle={() => updateSettings({ clipToOutline: !clipToOutline })}
