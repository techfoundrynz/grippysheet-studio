...
export function Dropdown({
  ...
  const getButtonClasses = () => {
    const baseClasses = "flex items-center gap-2 rounded-lg px-3 py-1.5 text-sm w-full transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500";
    
    if (disabled) {
      return `${baseClasses} bg-gray-800/30 text-gray-500 cursor-not-allowed`;
    }

    if (variant === 'ghost') {
      return `${baseClasses} bg-transparent text-gray-400 hover:text-white hover:bg-white/5`;
    }

    return `${baseClasses} bg-gray-900 text-gray-300 hover:bg-gray-800`;
  };
